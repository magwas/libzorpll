...
static GPrivate current_logtag_cache = G_PRIVATE_INIT(NULL);
...
z_log_release_cache(void)
{
  ...
  lc = g_private_get(&current_logtag_cache);
  if (lc)
    lc->used = 0;
...
z_log_trace_indent(gint dir)
{
  ...
  int *current_indent = g_private_get (&current_indent_key);
  const gchar *res;
  ...
  if (!current_indent)
    {
      ...
      *current_indent = 0;
      g_private_replace (&current_indent_key, current_indent);
    }
